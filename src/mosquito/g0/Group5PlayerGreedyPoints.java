...
public class Group5PlayerGreedyPoints extends mosquito.sim.Player  {
		...
		for (Light l : lights) {
			...
			if(greedyLights.containsKey(ml)) {
				if(greedyLights.get(ml) == null){
					...
					greedyLights.put(ml, location);
				}
				else{
					Point2D.Double destination = greedyLights.get(ml);
					if(Math.abs(destination.getX() - ml.getX()) < .5) {
						if(Math.floor(destination.getY()) > Math.floor(ml.getY())) {
							ml.moveDown();
						}else if(Math.floor(destination.getY()) < Math.floor(ml.getY())) {
